                        ...
                        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                            ...
                            if ($consulta->rowCount() > 0) {
                                ...
                                if (password_verify($pas, $usuario['upass'])) {
                                    session_start();
                                    $_SESSION['usuario'] = $usuario['unombre'];
                                    $_SESSION['utipo'] = $usuario['utipo']; // Guardar el tipo de usuario en la sesión

                                    // Redirigir basado en el tipo de usuario
                                    if ($usuario['utipo'] === 'admin') {
                                        header('Location: paneles/admin_dashboard.php');
                                    } else {
                                        header('Location: paneles/user_dashboard.php');
                                    }
                                    exit;
                                } else {
